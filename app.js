...
let catalogItemTypeFilter = "all";
let catalogItemTypeGroup = null;
let catalogSystemFilter = null;
...
const catalogFilterOptions = {
  ...
  spin: [
    { value: "", label: "전체", summaryLabel: "전체" },
    { value: "RIGHT SPIN", label: "우회전" },
    { value: "LEFT SPIN", label: "좌회전" },
    { value: "DUAL SPIN", label: "양회전" }
  ]
};
const catalogFilterDataAttrs = {
  series: "data-gear-series",
  kind: "data-gear-kind",
  subtype: "data-gear-subtype",
  system: "data-gear-system",
  type: "data-gear-type",
  spin: "data-gear-spin"
};
...
const renderCatalogStaticFilterOptions = () => {
  Object.entries(catalogFilterOptions).forEach(([key, options]) => {
    const dataAttr = catalogFilterDataAttrs[key];
    document.querySelectorAll(`#gearDropdownFilters [data-filter-group="${key}"] .catalog-filter-options, #gearDropdownFilters [data-filter-group="${key}"] .catalog-dropdown-menu, #gearDropdownFilters [data-gear-dependent="${key}"] .catalog-filter-options, #gearDropdownFilters [data-gear-dependent="${key}"] .catalog-dropdown-menu`).forEach(root => {
...
const deriveCatalogItemFilters = () => {
  if (!gearKindFilter) {
    catalogItemTypeFilter = "all";
    catalogItemTypeGroup = null;
  } else if (gearKindFilter === "tools") {
    catalogItemTypeFilter = "tools";
    catalogItemTypeGroup = null;
  } else if (gearKindFilter === "parts") {
    catalogItemTypeFilter = gearSubtypeFilter || "parts";
    catalogItemTypeGroup = gearSubtypeFilter ? gearTypeGroups[gearSubtypeFilter] || null : gearTypeGroups.parts;
  } else {
    catalogItemTypeFilter = gearKindFilter;
    catalogItemTypeGroup = null;
  }
  catalogSystemFilter = gearKindFilter === "bey" ? gearSystemFilter : null;
};
...
const visibleGearItems = () => {
  ...
  const useTypeFilter = catalogItemTypeFilter !== "all";
  const useMetalAttributeFilters = isMetalFightSeries(gearSeriesFilter);
  return items
    .filter(item => (!gearSeriesFilter || itemSeries(item) === gearSeriesFilter) && (!useTypeFilter || (catalogItemTypeGroup ? catalogItemTypeGroup.includes(item.type) : item.type === catalogItemTypeFilter)) && (!useMetalAttributeFilters || !catalogSystemFilter || item.structure === catalogSystemFilter) && (!useMetalAttributeFilters || matchesGearType(item)) && (!useMetalAttributeFilters || matchesGearSpin(item)) && itemMatchesSearch(item, query))
    .sort((a, b) => {
      if (useMetalAttributeFilters && a.type === "bey" && b.type === "bey") return beySerialNumber(a) - beySerialNumber(b);
      if (useMetalAttributeFilters && catalogItemTypeFilter === "wheel" && catalogItemTypeGroup) return (wheelTypeOrder[a.type] ?? 99) - (wheelTypeOrder[b.type] ?? 99);
      return 0;
    });
};
...
const overviewStructureGroups = [
  {
    series: "탑블레이드",
    structures: [
      { title: "초기 시스템", desc: "", parts: [], descriptions: [] },
      { title: "스핀기어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "네오스핀기어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "엔진기어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "헤비메탈 시스템", desc: "", parts: [], descriptions: [] }
    ]
  },
  {
    series: "메탈베이블레이드",
  ...
  },
  {
    series: "베이블레이드 버스트",
    structures: [
      { title: "싱글 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "듀얼 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "갓 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "초제트 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "진검 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "슈퍼킹 레이어 시스템", desc: "", parts: [], descriptions: [] },
      { title: "DB 레이어 시스템", desc: "", parts: [], descriptions: [] }
    ]
  },
  {
    series: "베이블레이드 X",
    structures: [
      { title: "베이직 라인", desc: "", parts: [], descriptions: [] },
      { title: "유니크 라인", desc: "", parts: [], descriptions: [] },
      { title: "커스텀 라인", desc: "", parts: [], descriptions: [] }
    ]
  }
];
...
const overviewSeries = overviewStructureGroups.map(group => group.series);
const currentOverviewStructures = () => overviewStructureGroups[overviewSeriesIndex]?.structures || [];
const currentOverviewStructure = () => currentOverviewStructures()[overviewStructureIndex] || currentOverviewStructures()[0] || { title: "", desc: "", parts: [], descriptions: [] };
const overviewSystemChipLabel = title => title.replace(/ 시스템$/, "");

const overviewGuideStructureControlsMarkup = () => `<div class="overview-structure-controls" aria-label="구조 정보 선택">
  <div class="overview-series-segments" role="group" aria-label="시리즈">
    ${overviewSeries.map((series, index) => `
      <button type="button" class="ui-tab-button overview-series-segment ${index === overviewSeriesIndex ? "active" : ""}" aria-pressed="${index === overviewSeriesIndex ? "true" : "false"}" data-overview-series="${index}">
        ${escapeHtml(series)}
      </button>
    `).join("")}
  </div>
  <div class="overview-system-chips" role="group" aria-label="시스템">
    ${currentOverviewStructures().map((structure, index) => `
      <button type="button" class="ui-tab-button overview-system-chip ${index === overviewStructureIndex ? "active" : ""}" aria-pressed="${index === overviewStructureIndex ? "true" : "false"}" data-overview-structure="${index}">
        ${escapeHtml(overviewSystemChipLabel(structure.title))}
      </button>
    `).join("")}
  </div>
</div>`;
...
function renderOverviewGuideStructure(scope = document) {
  ...
  const current = currentOverviewStructure();
  ...
  if (art && activeOverviewGuideTab === "structure") {
    const structureArt = overviewStructureArtMarkup();
    art.innerHTML = structureArt ? `<div class="overview-guide-art-stack overview-guide-art-single" aria-label="구조 도식">${structureArt}</div>` : "";
  }
...
const overviewGuideTabsMarkup = () => `<div class="ui-tabs overview-guide-tabs" role="tablist" aria-label="개요 주제">
  ${overviewGuideTabs.map(tab => `<button type="button" role="tab" aria-selected="${activeOverviewGuideTab === tab.id ? "true" : "false"}" class="ui-tab-button ${activeOverviewGuideTab === tab.id ? "active" : ""}" data-overview-guide-tab="${tab.id}">${tab.label}</button>`).join("")}
</div>`;
...
function renderOverviewGuide(scope = document) {
  ...
  if (activeOverviewGuideTab === "type") {
    art.innerHTML = `<div class="overview-guide-art-stack overview-guide-art-single" aria-label="타입 도식">${overviewGuideTypeArtMarkup()}</div>`;
  } else if (activeOverviewGuideTab === "spin") {
    art.innerHTML = `<div class="overview-guide-art-stack overview-guide-art-single" aria-label="회전 방향 도식">${overviewSpinArtMarkup()}</div>`;
  } else {
    const structureArt = overviewStructureArtMarkup();
    art.innerHTML = structureArt ? `<div class="overview-guide-art-stack overview-guide-art-single" aria-label="구조 도식">${structureArt}</div>` : "";
  }
...
const overviewAnimeTabs = [
  { id: "overview", label: "개요" },
  { id: "characters", label: "등장인물 및 베이" },
  { id: "episodes", label: "방영목록" }
];
let activeOverviewAnimeTab = "overview";
let activeAnimeEpisodeIndex = null;
let activeAnimeRegion = "kr";
let activeAnimeSeason = "metal-fight";
let activeAnimeEpisodeQuery = "";

const overviewAnimeTabsMarkup = () => `<div class="ui-tabs overview-anime-tabs" role="tablist" aria-label="애니메이션 정보">
  ${overviewAnimeTabs.map(tab => `<button type="button" role="tab" aria-selected="${activeOverviewAnimeTab === tab.id ? "true" : "false"}" class="ui-tab-button ${activeOverviewAnimeTab === tab.id ? "active" : ""}" data-overview-anime-tab="${tab.id}">${tab.label}</button>`).join("")}
</div>`;

const animeEmptyMarkup = message => `<p class="overview-anime-empty">${message}</p>`;
...
const animeOverviewMarkup = () => {
  ...
  return rows ? `<ul class="overview-anime-facts">${rows}</ul>` : animeEmptyMarkup("등록된 개요 정보가 없습니다.");
};
...
const animeCharactersMarkup = () => {
  ...
  return rows ? `<ul class="overview-anime-character-list">${rows}</ul>` : animeEmptyMarkup("등록된 등장인물 정보가 없습니다.");
};

const animeEpisodeDetailMarkup = () => {
  if (activeAnimeEpisodeIndex === null) return "";
  const episode = animeInfo.episodes[activeAnimeEpisodeIndex];
  if (!episode?.summary) return "";
  return `<section class="overview-anime-episode-detail">
    <h4>${escapeHtml(episode.no || "")} ${escapeHtml(episode.titles?.[activeAnimeRegion] || "")}</h4>
...
const animeEpisodesMarkup = () => {
  ...
  return `<section class="overview-anime-episodes" data-anime-table-mode="${tableMode}">
    ...
    ${animeEpisodeDetailMarkup()}
  </section>`;
};

const overviewAnimePanelMarkup = () => {
  if (activeOverviewAnimeTab === "characters") return animeCharactersMarkup();
  if (activeOverviewAnimeTab === "episodes") return animeEpisodesMarkup();
  return animeOverviewMarkup();
};

function rememberAnimeModalContext() {
  rememberModalContext("overview-anime", "OVERVIEW-ANIME", {
    animeTab: activeOverviewAnimeTab,
...
function renderOverviewAnime(root = document) {
  const tabs = root.querySelector("[data-overview-anime-tabs-slot]");
  const panel = root.querySelector("[data-overview-anime-panel]");
  if (!tabs || !panel) return;
  tabs.innerHTML = overviewAnimeTabsMarkup();
  panel.dataset.overviewAnimePanel = activeOverviewAnimeTab;
  panel.innerHTML = overviewAnimePanelMarkup();
  tabs.querySelectorAll("[data-overview-anime-tab]").forEach(button => button.addEventListener("click", event => {
    activeOverviewAnimeTab = event.currentTarget.dataset.overviewAnimeTab || "overview";
    activeAnimeEpisodeIndex = null;
    rememberAnimeModalContext();
    renderOverviewAnime(root);
  }));
  panel.querySelectorAll("[data-anime-region]").forEach(button => button.addEventListener("click", event => {
    activeAnimeRegion = releaseRegionLabels[event.currentTarget.dataset.animeRegion] ? event.currentTarget.dataset.animeRegion : "kr";
    activeAnimeEpisodeIndex = null;
    rememberAnimeModalContext();
    renderOverviewAnime(root);
  }));
  panel.querySelectorAll("[data-anime-season]").forEach(button => button.addEventListener("click", event => {
  ...
  const animeSearch = panel.querySelector("#animeEpisodeSearchInput");
  const syncAnimeSearchState = () => animeSearch?.classList.toggle("has-value", animeSearch.value.length > 0);
  const runAnimeSearch = () => {
    activeAnimeEpisodeQuery = animeSearch?.value.trim() || "";
    activeAnimeEpisodeIndex = null;
    syncAnimeSearchState();
    rememberAnimeModalContext();
    renderOverviewAnime(root);
    const nextInput = root.querySelector("#animeEpisodeSearchInput");
    nextInput?.focus();
    nextInput?.setSelectionRange(nextInput.value.length, nextInput.value.length);
  };
  animeSearch?.addEventListener("input", runAnimeSearch);
  animeSearch?.addEventListener("keydown", event => {
    if (event.key !== "Enter") return;
    event.preventDefault();
    runAnimeSearch();
  });
  animeSearch?.closest(".release-search-box")?.querySelector(".search-icon")?.addEventListener("click", runAnimeSearch);
  syncAnimeSearchState();
  panel.querySelectorAll("[data-anime-episode-index]").forEach(row => {
    const openEpisode = () => {
      const index = Number(row.dataset.animeEpisodeIndex);
      if (!animeInfo.episodes[index]?.summary) return;
      activeAnimeEpisodeIndex = index;
      renderOverviewAnime(root);
    };
    row.addEventListener("click", openEpisode);
    row.addEventListener("keydown", event => {
      if (event.key !== "Enter" && event.key !== " ") return;
      event.preventDefault();
      openEpisode();
...
function openOverviewAnimeDetail(options = {}) {
  activeOverviewAnimeTab = overviewAnimeTabs.some(tab => tab.id === options.animeTab) ? options.animeTab : "overview";
  ...
  const content = document.querySelector("#modalContent");
  content.innerHTML = `<div class="modal-inner overview-anime-modal">
    <div class="modal-info overview-anime-info">
      <div class="overview-title-row">
        <h3 class="overview-release-title">애니메이션</h3>
      </div>
      <div class="modal-body-block">
        <div data-overview-anime-tabs-slot></div>
        <section class="overview-anime-panel" data-overview-anime-panel></section>
      </div>
    </div>
  </div>`;
  renderOverviewAnime(content);
  rememberAnimeModalContext();
  window.location.hash = "OVERVIEW-ANIME";
  if (!modal.open) modal.showModal();
}
...
document.querySelectorAll("[data-overview-anime-open]").forEach(button => {
  button.addEventListener("click", () => openOverviewAnimeDetail());
});
...
const renderGearSubtypeOptions = () => {
  ...
  const optionMarkup = `
    <button type="button" class="ui-dropdown-item ${gearSubtypeFilter ? "" : "active"}" data-gear-subtype="" data-summary-label="전체">전체</button>
    ${options.map(option => `<button type="button" class="ui-dropdown-item ${gearSubtypeFilter === option.value ? "active" : ""}" data-gear-subtype="${option.value}">${option.label}</button>`).join("")}
  `;
...
if (window.location.hash) {
  ...
  else if (hashId === "OVERVIEW-ANIME") openOverviewAnimeDetail(restoredOptions);
