...
const expectedBurstAirDates14To26 = [
  "2016-08-29",
  "2016-09-05",
  "2016-09-12",
  "2016-09-19",
...
const expectedSuperkingEpisodes = [
  ...
  ["폭풍같은 레이지 롱기누스!", "2020-10-07", "2020-09-30 추석 연휴 휴방"],
...
const expectedDbEpisodes = [
  ["마왕! 다이너마이트 벨리알!", "2021-07-07"],
  ["벨리알 대 롱기누스", "2021-07-14"],
  ["베이의 무덤! 마계의 문!", "2021-07-21"],
  ["질풍노도! 사이클론 라그나로크!", "2021-07-28"],
  ["모드 체인지! 하이 앤드 로우!", "2021-08-04"],
  ["경쟁자와 아미고", "2021-08-11"],
  ["마계극장! 벨 대 강산!", "2021-08-18"],
  ["역습! 다이너마이트 보머!", "2021-08-25"],
  ["마왕! 세계로 나가다!", "2021-09-01"],
  ["베이의 품격! 배니쉬 파브닐!", "2021-09-08"],
  ["또 하나의 발키리!", "2021-09-15"],
  ["더 강력해진 벨리알!", "2021-09-22"],
  ["역전 작전! 벨리알 대 파브닐!", "2021-09-29"],
  ["최강 검! 세이비어 발키리!", "2021-10-06"],
  ["신룡의 포효! 로어 바하무트!", "2021-10-13"],
  ["배틀 각성! 발키리 대 발키리!", "2021-10-20"],
  ["하늘을 나는 다이너마이트 배틀 투어!", "2021-10-27"],
  ["성지 격돌! 베이학교의 결투!", "2021-11-03"],
  ["새로운 별들의 전쟁! 벨 대 라샤드!", "2021-11-10"],
  ["슈의 아스트랄 스프리건!", "2021-11-17"],
  ["마왕의 기습! 벨 대 프리!", "2021-11-24"],
  ["도깨비 섬의 배틀 로열!", "2021-12-01"],
  ["하인이 된 마왕!", "2021-12-08"],
  ["정열의 춤! 마그마 이프릿!", "2021-12-15"],
  ["여덟 개의 드래곤! 길티 롱기누스!", "2021-12-22"],
  ["도깨비 섬의 특별 배틀!", "2021-12-29"],
  ["마왕의 진격! MVP는?", "2022-01-05"],
  ["파워대결! 다이너마이트 배틀!", "2022-01-12"],
  ["마왕의 부활! 데인저러스 벨리알!", "2022-01-19"],
  ["빛의 패왕 강림!", "2022-01-26"],
  ["둥근 빛! 그레이티스트 라파엘!", "2022-02-09", "2022-02-02 설 연휴 휴방"],
  ["마왕 대 패왕! 최강팀 결정전!", "2022-02-16"],
  ["역전의 역전! 바사라의 역습!", "2022-02-23"],
  ["3회전! 배틀 로열!", "2022-03-02"],
  ["마왕, 팀을 나가다?!", "2022-03-16", "2022-03-09 대통령 선거일 휴방"],
  ["BC솔 최강자들의 격돌!", "2022-03-23"],
  ["추의 지옥! 그레이티스트 펜듈럼!", "2022-03-30"],
  ["최고의 파트너! 얼티밋 발키리!", "2022-04-06"],
  ["파워 폭발! 풀 커스텀 베이 런처!", "2022-04-13"],
  ["하이 앤드 로우! 하이브리드 스타디움!", "2022-04-20"],
  ["불사조! 프로미넌스 피닉스!", "2022-04-27"],
  ["데인저러스, 무조건 돌파해!", "2022-05-04"],
  ["불꽃의 향연! 프로미넌스 셰이커!", "2022-05-11"],
  ["증오의 실드 트랩!", "2022-05-18"],
  ["퍼펙트 기어! 불사조를 쫓아!", "2022-05-25"],
  ["격돌! 패왕 대 불사조!", "2022-06-08", "2022-06-01 지방선거일 휴방"],
  ["웰컴! 마계의 문!", "2022-06-08", "46화와 연속 방영"],
  ["최강의 대결! 익스트림 배틀!", "2022-06-15"],
  ["대격돌! 숙명의 라이벌전!", "2022-06-22"],
  ["공감! 강산 대 라샤드!", "2022-06-29"],
  ["데인저러스 대 그레이티스트!", "2022-07-06"],
  ["폭발! 파이널 배틀!", "2022-07-13"]
];

const expectedRows = episodes => episodes.map(([title, airDate, note = ""], index) => [
  String(index + 1) + "화",
  title,
  "",
  airDate,
...
const expectedDbRows = expectedRows(expectedDbEpisodes);
...
const expectedDbIds = expectedDbEpisodes.map((_, index) => "BURST-DB-EPISODE-" + (index + 1));

test("버스트 14화부터 26화까지 한국 방영일은 주간 순서를 유지한다", () => {
  const sourceEpisodes = animeInfo.episodes.filter(episode => episode.season === "burst").slice(13, 26);
  const runtimeEpisodes = runtimeAnime.episodes.filter(episode => episode.season === "burst").slice(13, 26);
  const searchEntries = runtimeSearch.search.filter(entry => entry[4] === "burst").slice(13, 26);

  assert.deepEqual(sourceEpisodes.map(episode => episode.airDates.kr), expectedBurstAirDates14To26);
  assert.deepEqual(runtimeEpisodes.map(episode => episode.airDates.kr), expectedBurstAirDates14To26);
  assert.deepEqual(searchEntries.map(entry => entry[7]), expectedBurstAirDates14To26);
});
...
test("슈퍼킹 방영목록 52건은 제공된 한국 제목과 기존 방영일 순서를 유지한다", () => {
  ...
  assert.equal(animeInfo.episodes[firstIndex + episodes.length].season, "burst-db");
  ...
  assert.ok(episodes.every(episode => episode.titles.jp === "" && episode.airDates.jp === ""));
});
...
test("DB 방영목록 52건은 제공된 한국 제목과 방영일 순서를 유지한다", () => {
  const episodes = animeInfo.episodes.filter(episode => episode.season === "burst-db");
  assert.equal(episodes.length, 52);
  assert.deepEqual(episodes.map(episodeTuple), expectedDbRows);

  const firstIndex = animeInfo.episodes.findIndex(episode => episode.season === "burst-db");
  assert.equal(animeInfo.episodes[firstIndex - 1].season, "burst-superking");
  assert.equal(animeInfo.episodes[firstIndex + episodes.length].season, "beyblade-x");
  assert.equal(episodes[1].titles.kr, "벨리알 대 롱기누스");
  assert.equal(episodes[6].titles.kr, "마계극장! 벨 대 강산!");
  assert.equal(episodes[9].titles.kr, "베이의 품격! 배니쉬 파브닐!");
  assert.equal(episodes[32].titles.kr, "역전의 역전! 바사라의 역습!");
  assert.deepEqual(episodes.slice(45, 47).map(episode => episode.airDates.kr), ["2022-06-08", "2022-06-08"]);
  assert.equal(episodes[47].airDates.kr, "2022-06-15");
  assert.ok(episodes.every(episode => episode.titles.jp === "" && episode.airDates.jp === ""));
});

test("DB 에피소드 주소·검색·레지스트리가 1화부터 52화까지 생성된다", () => {
  const runtimeEpisodes = runtimeAnime.episodes.filter(episode => episode.season === "burst-db");
...
test("한국어 방영목록은 공식 한국어 제목이 확인된 회차만 포함한다", () => {
  const japaneseOnlyEpisodes = animeInfo.episodes.filter(episode => episode.titles.jp && !episode.titles.kr);
  assert.deepEqual(japaneseOnlyEpisodes, []);
});
